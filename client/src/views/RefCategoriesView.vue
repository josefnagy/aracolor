...
import { useRefCategories } from '../composables/useRefCategories.js';
import {
  Plus as PlusIcon,
  Image as ImageIcon,
...
const { categoryList: rawCategoryList, images, carouselCountByCategory, loadData } = useRefImages();
const { createCat, renameCat, deleteCat, reorderCats } = useRefCategories();
...
const categoryList = computed(() => {
  return rawCategoryList.value.map(cat => {
    ...
    if (cat.heroImageId && images.value[cat.heroImageId]) {
      heroSrc = images.value[cat.heroImageId].thumb || images.value[cat.heroImageId].src;
    }
    return { ...cat, heroSrc };
  });
});
